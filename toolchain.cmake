...
IF (SJR_OS_LINUX)
    IF (SJR_OS_ANDROID)
        SET(SJR_ANDROID_LINK "-fuse-ld=${SJR_3RD_NDK}/toolchains/arm-linux-androideabi-4.9/prebuilt/linux-x86_64/bin/arm-linux-androideabi-ld.gold")
    ELSE ()
        SET(SJR_LINUX_LINK "-fuse-ld=ld")
    ENDIF ()
ELSEIF (SJR_OS_WINDOWS)
    IF (SJR_OS_ANDROID)
        SET(SJR_ANDROID_LINK "-fuse-ld=${SJR_3RD_NDK}/toolchains/arm-linux-androideabi-4.9/prebuilt/windows-x86_64/bin/arm-linux-androideabi-ld.gold")
    ELSE ()
...
# 清空 flags 初始化
IF (SJR_OS_ANDROID)
    SET(CMAKE_C_FLAGS_INIT "--target=armv7-none-linux-androideabi -std=c11 -fPIC -mthumb -fvisibility=hidden --sysroot=${SJR_3RD_NDK}/sysroot ${SJR_FLAGS}")
    SET(CMAKE_CXX_FLAGS_INIT "--target=armv7-none-linux-androideabi -std=c++17 -fPIC -mthumb -fvisibility=hidden -fno-rtti --sysroot=${SJR_3RD_NDK}/sysroot ${SJR_FLAGS}")
ELSEIF (SJR_OS_LINUX)
    SET(CMAKE_C_FLAGS_INIT "-std=c11 ${SJR_FLAGS} -fPIC")
    SET(CMAKE_CXX_FLAGS_INIT "-std=c++17 -fno-rtti -stdlib=libstdc++ ${SJR_FLAGS} -fPIC")
ELSEIF (SJR_OS_WINDOWS)
    SET(CMAKE_C_FLAGS_INIT "-std=c11 ${SJR_FLAGS}")
    SET(CMAKE_CXX_FLAGS_INIT "-std=c++17 ${SJR_FLAGS}")
ENDIF ()
...
# 重置可执行文件生成规则
IF (SJR_OS_ANDROID)
    SET(CMAKE_C_LINK_EXECUTABLE "<CMAKE_C_COMPILER> ${CMAKE_C_FLAGS_INIT} ${SJR_ANDROID_LINK} -fPIE -Wl,--no-undefined <OBJECTS> -o <TARGET> <LINK_LIBRARIES>")
    SET(CMAKE_CXX_LINK_EXECUTABLE "<CMAKE_CXX_COMPILER> ${CMAKE_CXX_FLAGS_INIT} ${SJR_ANDROID_LINK} -fPIE -Wl,--no-undefined <OBJECTS> -o <TARGET> <LINK_LIBRARIES>")
ELSEIF (SJR_OS_LINUX)
